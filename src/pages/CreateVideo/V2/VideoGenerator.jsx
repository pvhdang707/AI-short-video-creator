...
const VideoGenerator = ({
  ...
  const handleGenerateFromScript = async () => {
    try {
      ...
      const videoBlob = await generateVideoFromScript(
        ffmpeg,
        script,
        (progress) => {
  ...
  const handleTestFFmpeg = async () => {
    try {
      ...
      const testImageUrl =
        "https://via.placeholder.com/854x480/000000/FFFFFF?text=Test+Image";

      const videoBlob = await createSimpleVideo(
        ffmpeg,
        testImageUrl,
        3,
        (progress) => {
  ...
  const handleSaveToCloud = async () => {
    ...
    try {
      ...
      const uploadResponse = await fetch(
        "http://localhost:8000/api/v1/storage/upload-video",
        {
          method: "POST",
          body: formData,
        }
  ...
  const updateVideoLinkInDB = async (downloadUrl) => {
    ...
    try {
      ...
      if (!scriptId) {
        toast.error(
          "Unable to find script ID to update video! Please recreate the video."
        );
        return;
      }
      ...
      try {
        ...
        console.error(
          "Script không tồn tại hoặc lỗi khi kiểm tra:",
          checkError
        );
        toast.error(
          "Script does not exist in database! Please recreate the script."
        );
        return;
      }
      ...
      toast.error(
        "Error updating video to DB: " + (err?.detail || err?.message || err)
      );
    }
  };
          ...
          <div className="relative p-6">
            <div className="flex items-center space-x-4 mb-4">
              ...
              <div className="flex-1">
                <div className="flex justify-between items-center mb-3">
                  <div>
                    ...
                    <div className="text-sm text-slate-400 mt-1">
                      ...
                      {progress >= 10 && progress < 30 && "Loading scenes..."}
                      {progress >= 30 && progress < 60 && "Processing scenes..."}
                      {progress >= 60 && progress < 80 && "Applying effects..."}
                      {progress >= 80 && progress < 85 && "Preparing video..."}
                      {progress >= 85 && progress < 95 && "Rendering video..."}
                      {progress >= 95 && progress < 98 && "Finalizing..."}
                      {progress >= 98 && progress < 100 && "Reading file..."}
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-slate-400 mt-1">
                      ...
                      {progress >= 10 && progress < 30 && "Loading..."}
                      {progress >= 30 && progress < 60 && "Processing..."}
                      {progress >= 60 && progress < 80 && "Effects..."}
                      {progress >= 80 && progress < 85 && "Preparing..."}
                      {progress >= 85 && progress < 95 && "Rendering..."}
                      {progress >= 95 && progress < 98 && "Finalizing..."}
                      {progress >= 98 && progress < 100 && "Reading..."}
                      {progress >= 100 && "Done!"}
                    </div>
                  </div>
                </div>
                ...
                {/* Main Progress Bar */}
                <div className="w-full h-4 bg-slate-700/50 rounded-full overflow-hidden mb-2">
                  <div
                ...
                <div className="flex justify-between items-center text-xs text-slate-400">
                  <div className="flex items-center space-x-1">
                    <div className={`w-2 h-2 rounded-full ${progress >= 10 ? 'bg-green-400' : 'bg-slate-600'}`}></div>
                    <span>Load</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <div className={`w-2 h-2 rounded-full ${progress >= 30 ? 'bg-green-400' : 'bg-slate-600'}`}></div>
                    <span>Process</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <div className={`w-2 h-2 rounded-full ${progress >= 60 ? 'bg-green-400' : 'bg-slate-600'}`}></div>
                    <span>Effects</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <div className={`w-2 h-2 rounded-full ${progress >= 85 ? 'bg-green-400' : 'bg-slate-600'}`}></div>
                    <span>Render</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <div className={`w-2 h-2 rounded-full ${progress >= 95 ? 'bg-green-400' : 'bg-slate-600'}`}></div>
                    <span>Finish</span>
                  </div>
                </div>
              </div>
